...
Build a one-page React + TypeScript marketing site for Automatic Door Solutions using a static architecture optimised for low-cost hosting on Cloudflare Pages. The implementation will prioritise a premium visual identity, strong service storytelling, project showcase via carousel + gallery, and conversion-focused contact links (email + phone) with a clean upgrade path to a full contact form later.
...
1. Set up global design tokens (CSS variables) for typography, colour, spacing, elevation, and motion to enforce a consistent premium visual system.
...
1. Ensure fully responsive behaviour for desktop and mobile (including button sizing, readable typography scales, and section spacing).
...
1. Optimise images and loading behaviour to preserve performance on mobile.
...
- public/ - optimised static media assets
...
2. Responsive QA at common breakpoints (mobile, tablet, desktop), including carousel touch/keyboard behaviour.
...
- Contact behaviour: display-only email/phone now, explicit extension path for form later.
